...
INSERT OR IGNORE INTO bus_events_catalog(
  ...
  ('ECHO_REQUEST_NOT_FOUND','bus.response.echo_request_not_found','BUS_MESSAGE','RESPONSE request-block bus_id not found in bus_messages','RESPONSE request-block bus_id not found in bus_messages',1),
  ...
  ('LANE_MISMATCH',NULL,'BUS_MESSAGE','RESPONSE lane_id does not match REQUEST lane_id referenced by request block bus_id','RESPONSE lane_id does not match REQUEST lane_id referenced by request block bus_id',1),
  ('MISSING_ECHO_REQUEST_BUS_ID',NULL,'BUS_MESSAGE','RESPONSE is missing request block bus_id','RESPONSE is missing request block bus_id',1),
  ('REQUEST_ID_MISMATCH',NULL,'BUS_MESSAGE','RESPONSE request_id does not match REQUEST request_id referenced by request block bus_id','RESPONSE request_id does not match REQUEST request_id referenced by request block bus_id',1);
