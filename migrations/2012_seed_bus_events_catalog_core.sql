...
INSERT OR IGNORE INTO bus_events_catalog(
  ...
  ('AUTO_FINALIZE_ACK','INFO','BUS_MESSAGE','AUTO_FINALIZE','["q_state", "reason"]','["criteria.match.claim_age_sec_gt", "criteria.match.in_state", "criteria.match.lane_id", "criteria.match.msg_type", "criteria.match.state", "criteria.match.to_owner_id", "criteria.target_q_state", "dry_run", "incident_id", "sample_bus_ids", "transition.q_state.from", "transition.q_state.to", "transition.done_at.from", "transition.done_at.to"]','Auto-finalize ACK executed safely for one message (bus_id).'),
