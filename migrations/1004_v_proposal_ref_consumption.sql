...
WITH consumers AS (
  SELECT
    json_extract(r.bus_json, '$.message.contents.proposal_ref.bus_id') AS proposal_ref_bus_id,
    ...
    r.request_id,
    ROW_NUMBER() OVER (
      PARTITION BY json_extract(r.bus_json, '$.message.contents.proposal_ref.bus_id')
  ...
  WHERE r.msg_type = 'REQUEST'
    ...
    AND json_extract(r.bus_json, '$.message.contents.proposal_ref.bus_id') IS NOT NULL
)
SELECT
