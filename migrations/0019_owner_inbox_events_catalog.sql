...
CREATE TABLE IF NOT EXISTS owner_inbox_events_catalog (
  ...
  required_fields TEXT,        -- JSON array of required owner_inbox_events columns for this event (optional)
  required_data_keys TEXT,     -- JSON array of required JSON key paths under owner_inbox_events.data
  optional_data_keys TEXT,     -- JSON array of optional JSON key paths under owner_inbox_events.data
  notes TEXT
);

CREATE INDEX IF NOT EXISTS idx_owner_inbox_events_catalog_sev ON owner_inbox_events_catalog(severity);
