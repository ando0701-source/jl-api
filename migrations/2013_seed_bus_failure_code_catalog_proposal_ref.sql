...
INSERT OR IGNORE INTO bus_failure_code_catalog(
  ...
  ('proposal_ref_target_terminal_mismatch','ERROR','RESTART_FROM_JL_PROPOSAL','BUS_MESSAGE','2PLT_40_TARGET_RESOLUTION_PHASE1_ANNEX','2PLT_40_TARGET_RESOLUTION_PHASE1_ANNEX','TR_P1A_004_TARGET_TERMINAL_IS_PROPOSAL','AUDIT_CHECK_E10_ENQUEUE_PRECHECK_REJECTED',NULL,'["proposal_ref_bus_id", "target_state", "target_out_state"]','["bus_id", "op_id", "flow_owner_id", "lane_id", "request_id"]','PHASE1A',1,'proposal_ref.bus_id must target a JL_PROPOSAL response whose terminal is PROPOSAL','A JL_PROPOSAL response ending in UNRESOLVED or ABEND cannot be committed or rejected.'),
...
UPDATE bus_failure_code_catalog SET
  ...
  required_detail_keys='["proposal_ref_bus_id", "target_state", "target_out_state"]', optional_detail_keys='["bus_id", "op_id", "flow_owner_id", "lane_id", "request_id"]', phase_introduced='PHASE1A', is_terminal=1,
  message_template='proposal_ref.bus_id must target a JL_PROPOSAL response whose terminal is PROPOSAL', notes='A JL_PROPOSAL response ending in UNRESOLVED or ABEND cannot be committed or rejected.'
