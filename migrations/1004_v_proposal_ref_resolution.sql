...
SELECT
  ...
  CAST(json_extract(r.bus_json, '$.message.contents.PROPOSAL.bus_id') AS TEXT) AS proposal_ref_bus_id,
  CAST(json_extract(r.bus_json, '$.message.contents.PROPOSAL.doc_id') AS TEXT) AS proposal_ref_source_op_id,
  CAST(json_extract(r.bus_json, '$.message.contents.PROPOSAL.op_id') AS TEXT) AS proposal_ref_source_terminal,
  'CONTENT_BLOCK_BUS_ID' AS proposal_ref_resolution_mode,
  ...
  CAST(json_extract(p.bus_json, '$.message.contents.JL_PROPOSAL.bus_id') AS TEXT) AS target_request_bus_id,
  ...
  CASE
    ...
    WHEN COALESCE(TRIM(CAST(json_extract(p.bus_json, '$.message.contents.JL_PROPOSAL.bus_id') AS TEXT)), '') = '' THEN 'PROPOSAL_REF_ORIGIN_REQUEST_INVALID'
...
LEFT JOIN bus_messages p
  ON p.bus_id = CAST(json_extract(r.bus_json, '$.message.contents.PROPOSAL.bus_id') AS TEXT)
...
LEFT JOIN bus_messages c
  ON c.bus_id = (
    ...
    WHERE prior_req.msg_type = 'REQUEST'
      ...
      AND TRIM(CAST(json_extract(prior_req.bus_json, '$.message.contents.PROPOSAL.bus_id') AS TEXT)) = TRIM(CAST(json_extract(r.bus_json, '$.message.contents.PROPOSAL.bus_id') AS TEXT))
