...
LEFT JOIN bus_messages c
  ON c.bus_id = (
    ...
    WHERE prior_req.msg_type = 'REQUEST'
      ...
      AND TRIM(CAST(json_extract(prior_req.bus_json, '$.message.contents.proposal_ref.bus_id') AS TEXT)) = TRIM(CAST(json_extract(r.bus_json, '$.message.contents.proposal_ref.bus_id') AS TEXT))
